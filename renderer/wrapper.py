...
def build_scene(points, cfgs):

    views, pose= [cfgs.POINT.view], cfgs.POINT.pose
    xyz, features = points[..., :3], points[..., 3:]
    xyz *= cfgs.POINT.scale
    xyz = rotate_points(xyz, pose[3:])
    xyz = xyz + np.array(pose[:3])

    scene_dict = create_environment(views=views, **cfgs.RENDERER)
    scene_dict.update(pointcloud_1=create_cloud(np.concatenate((xyz, features), axis=-1), **cfgs.POINT))
